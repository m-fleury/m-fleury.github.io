...
(Better [[file:../figures/sat/sc2021-isasat-kissat-cleanmaple.pdf][PDF]] version). The conclusion is that IsaSAT would not have been the
slowest solver submitted at the SAT Competition 2021 (if I had been
able to make IsaSAT work on Starexec). It is not a great result, but
gives an idea how far you can get without any inprocessing with a
verified SAT solver.
